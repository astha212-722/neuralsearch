...
def search_google(query, n_results=10):
    try:
        params = {"q": query, "num": n_results, "api_key": SERPAPI_KEY}
        search = GoogleSearch(params)
        results = search.get_dict()
        organic = results.get("organic_results", [])
        return [{"title": r["title"], "snippet": r.get("snippet",""), "url": r["link"], "source": "Google"} for r in organic[:n_results]]
    except Exception as e:
        print(f"  Google error: {e}")
        return []

def search_arxiv(query, n_results=10):
    try:
        client = arxiv.Client()
        search = arxiv.Search(query=query, max_results=n_results, sort_by=arxiv.SortCriterion.Relevance)
        results = list(client.results(search))
        return [{"title": r.title, "snippet": r.summary[:200], "url": r.entry_id, "source": "ArXiv"} for r in results]
    except Exception as e:
        return []

def search_web(query, n_results=10):
    print("  Trying Google...")
